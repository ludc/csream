 ...
 local BREAM, parent = torch.class('csream.BREAM', 'nn.Module')
...
function BREAM:__init(input_dimension,output_dimension,N,length,coef_l1,costs,internal_cell)
  parent.__init(self)
...
function BREAM:updateGModule(cuda)
  local input_x=nn.Identity()()
  local initial_vector=self.constant_module(input_x)
  
  self.nmodule_policies={}
  for i=1,self.length do
    self.nmodule_policies[i]=nn.Sequential():add(self.module_policies[i]):add(nn.Sigmoid()):add(self.__mul:clone()):add(self.__add:clone()):add(csream.MyL1Penalty(torch.mul(self.weights,self.l1[i]))):add(nn.ReinforceBernoulli())
  end
...
function BREAM:reset(stdv,bias)
...
function BREAM:getPoliciesOutput()
  local outputs={}
  for i=1,self.length do 
...
function BREAM:getPoliciesProbas()
  local outputs={}
  for i=1,self.length do 
...
function BREAM:reinforce(reward)
  for i=1,self.length do
...
--- input is mu, sigma
function BREAM:updateOutput(input)
  self.module:updateOutput(input)
  self.output=self.module.output
  return self.output
end

function BREAM:updateGradInput(input, gradOutput)
  self.module:updateGradInput(input,gradOutput)
  self.gradInput=self.module.gradInput
  return(self.gradInput)
end

function BREAM:accGradParameters(input, gradOutput, scale)
  self.module:accGradParameters(input,gradOutput,scale)
end

function BREAM:zeroGradParameters()
   self.module:zeroGradParameters()
end

function BREAM:updateParameters(learningRate)
   self.module:updateParameters(learningRate)
end

-- we do not need to accumulate parameters when sharing
BREAM.sharedAccUpdateGradParameters = BREAM.accUpdateGradParameters

function BREAM:__tostring__()
